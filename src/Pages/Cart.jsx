...
import CartToral from "../components/CartToral";
const Cart = () => {
          ...
          return (
            ...
            >
              ...
              <input
                ...
                min={1}
                className="border max-w-10 sm:max-w-20 px-1 sm:px2 py1"
              />
              <img
                onClick={() => updateQuantity(item._id, item.size, 0)}
                src={assets.bin_icon}
                alt=""
                className="w-4 sm-w5 cursor-pointer mr-5"
              />
            </div>
          );
        })}
...
<button onClick={()=>navigate('placeOrder')} className="bg-black text-white text-sm  px-8 py-3">PROSEED TO CHECKOUT</button>
          </div>
        </div>
      </div>
    </div>
